                        ...
                        if (obj2Check.enabled && obj2Check.collidable) {
                            if (obj2Check !== object) { //check against itself?
                                if (object.type === 'LaserShoot') {
                                    if (obj2Check.type === 'Alian' && object.isColid(obj2Check)) {
                                        object.enabled = false;
                                        obj2Check.enabled = false;
                                        this.dispacher.dispatchEvent(new Event("AnAlianKilled"));
                                    }
                                    else if (obj2Check.type === 'Barrier' && object.isColid(obj2Check)) {
                                        this.Beep();
                                    }
...
export class Alian extends Rectangle {
    constructor(config) {
        ...
        if (typeof config !== 'undefined') {
            ...
            if (typeof config.race !== 'undefined')
                this.race = config.race;
            if (typeof config.dispacher !== 'undefined')
                this.dispacher = config.dispacher;
            if (typeof config.color !== 'undefined')
                this.color = config.color;
            if (typeof config.x !== 'undefined')
                this._x = config.x;
    ...
    SetupEvents() {
        this.dispacher.addEventListener('AnAlianHitTheWall', (event) => {
            if (event.detail.wall === 'left')
                this.ChangeDir(dir.right);
            else if (event.detail.wall === 'right')
                this.ChangeDir(dir.left);
        });
    }
    Update() {
        this.UpdateLocation();
...
export class LaserShoot extends Rectangle {
    ...
    isColid(obj2Check) {
        return (this.x < obj2Check.x + obj2Check.width &&
